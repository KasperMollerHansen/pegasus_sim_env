...
public:
    OffboardControl() : Node("offboard_control"), armed_(false)
    {
        ...
        // Create subscriber for cmd_vel_subscriber
        cmd_vel_subscriber_ = this->create_subscription<geometry_msgs::msg::Twist>(
            "/cmd_vel", qos_profile, [this](const geometry_msgs::msg::Twist::SharedPtr msg) {
                RCLCPP_INFO(this->get_logger(), "Received velocity command");
...
private:
    ...
    void publish_trajectory_setpoint(const geometry_msgs::msg::Twist::SharedPtr &received_msg);
 ...
 */
void OffboardControl::publish_trajectory_setpoint(const geometry_msgs::msg::Twist::SharedPtr &received_msg)
{
    TrajectorySetpoint msg{};

    msg.position = {NAN, NAN, NAN};
    // Transform position: [x, y, z] -> [y, x, -z]
    msg.velocity = {static_cast<float>(received_msg->linear.y),  // y
        static_cast<float>(received_msg->linear.x),  // x
